...
void main() {
  test('returns default settings when repository is empty', () async {
    final settingsRepository = _MemorySettingsRepository(null);
    ...
    final settings = await container.read(currentAppSettingsProvider.future);

    expect(settings.themeMode, AppThemePreference.system);
    expect(settings.weightUnit, WeightUnit.kg);
    expect(settings.preferredGraphRange, ProgressRange.last30Days);
  });

  test('persists theme mode and weight unit changes', () async {
    ...
    await container.read(settingsControllerProvider).saveThemeMode(AppThemePreference.dark);
    await container.read(settingsControllerProvider).saveWeightUnit(WeightUnit.lb);

    final updated = await container.read(currentAppSettingsProvider.future);
